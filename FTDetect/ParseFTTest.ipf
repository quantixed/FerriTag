...
Menu "Macros"
	"FerriTag distribution...",  LoadTIFFsAndComDet()
End
...
Function LoadTIFFsAndComDet()
	...
	Variable FileLoop, pixelSize, nWaves, i
	...
	for (FileLoop = 0; FileLoop < nFiles; FileLoop += 1)
		...
		// also return the number of particles
		nGT = ProcessMask(maskMat)
		...
		pixelSize = 0.07 * (0.2 / CheckScaleBar(ParentImgMat))^2 //sb is 200 nm
		sbPxSize[FileLoop] = pixelSize // in um3
		//scale volumeWave result for this row
		volumeWave[fileLoop][] *= pixelSize
...
Function ProcessMask(maskMat)
	Wave maskMat
	// particles here are false detection
	maskMat[][] = (maskMat[p][q] <= 128) ? 1 : maskMat[p][q] // corresponds to cytoplasm
	// particles here are real particles
	maskMat[][] = (maskMat[p][q] >= 129) ? 2 : maskMat[p][q] // corresponds to membrane
	// there will be no 3 or 0 (scale bar zone or extracellular region respectively)
	Variable nGT = ceil((sum(maskMat) - numpnts(maskMat)) / 9 )
	return nGT
End
...
Function CheckScaleBar(matB)
	...
	Make/O/I/N=(7)/FREE sbStartSeq={0,255,255,255,255,255,0}
	Make/O/I/N=(8)/FREE sbEndSeq={0,255,255,255,255,255,255,0}
	
	Variable vStart,vEnd, vLen
	
	MatrixOP/O/FREE sbWave = col(matB,1008)
...
///	@param	nGT	number of ground truth particles
Function CalculateDensities(nGT)
	Variable nGT
	...
	MatrixOp/O volumeTotal = sumcols(volumeWave)
	MatrixOp/O densityWave = sumcols(countWave) / sumcols(volumeWave)
	// Print results to Notebook
	NewNotebook/F=0/N=summaryNotes
	text0 = num2str(countTotal[0][1])
	Notebook summaryNotes, text=text0, text=" particles were falsely detected.\r"
	text0 = num2str(countTotal[0][2])
	text1 = num2str(volumeTotal[0][1] + volumeTotal[0][2])
	Notebook summaryNotes, text=text0, text=" particles were detected in a total volume of ", text=text1, text=" µm^3.\r\r"
	text0 = num2str(nGT)
	Notebook summaryNotes, text="The total number of ground truth particles was ", text=text0, text=".\r"
	text0 = num2str(dimsize(countwave,0))
	Notebook summaryNotes, text="Across ", text=text0, text=" images.\r"
	
	MatrixOp/O/FREE tempW = col(countWave,2) / nGT
	WaveStats/Q tempW
	text0 = num2str(V_avg)
	text1 = num2str(V_sdev)
	Notebook summaryNotes, text="This is a detection rate of ", text=text0, text=" ± ", text=text1, text=".\r"
